...
contract curvePool_Swap{
    ...
    function filterSwap(bytes memory  exchangeData)  payable external returns(uint256) {
        address inputAddre;
       ...
       return exchange_Multiples(inputAddre,route,swap_params,amount,expected,pools,receiver);
    }
    ...
    function exchange_Multiples( address _inputAddre,address[9] memory _route,uint256[3][4] memory _swap_params,uint256 _amount,uint256 _expected,address[4] memory _pools,address _receiver)  internal  returns(uint256){
     ...
     if(_inputAddre == address(0)){
         return ICurveRouter(CURVE_SWAP).exchange_multiple{value: _amount}(_route,_swap_params,_amount,_expected,_pools,_receiver);
     }else{
         TransferHelper.safeApprove(_inputAddre,CURVE_SWAP,_amount);
         return ICurveRouter(CURVE_SWAP).exchange_multiple(_route,_swap_params,_amount,_expected,_pools,_receiver);
